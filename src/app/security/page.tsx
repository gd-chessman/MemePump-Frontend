...
export default function SecurityPage() {
  ...
  const [ggAuthPasswordError, setGgAuthPasswordError] = useState("")
  ...
  const handleCodeChange = (index: number, value: string) => {
    if (value.length <= 1 && /^\d*$/.test(value)) {
      ...
      if (value && index < 3) {
        const nextInput = document.getElementById(`password-code-${index + 1}`);
        nextInput?.focus();
      }
    }
  };
  ...
  const handleCodeKeyDown = (index: number, e: React.KeyboardEvent) => {
    if (e.key === "Backspace" && !verificationCode[index] && index > 0) {
      const prevInput = document.getElementById(`password-code-${index - 1}`);
      prevInput?.focus();
    }
  };
  ...
  const proceedWithGoogleAuth = async (inputPassword?: string) => {
    try {
      ...
      setGgAuthPasswordError("");
    } catch (error: any) {
      console.error("Error adding Google Authenticator:", error);
      setGgAuthPasswordError(t('security.invalid_password'));
    }
  }
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      ...
      setPasswordError(error.message || t('security.password_change_failed'));
    } finally {
  ...
  const handleSendCode = async () => {
    ...
    try {
      // TODO: Implement send code API call
      // await sendVerificationCode();
      setIsCodeSent(true);
    } catch (error: any) {
      setPasswordError(error.message || t('security.send_code_failed'));
    } finally {
    ...
    <div className="min-h-screen text-gray-900 dark:text-white p-4 transition-colors duration-300">
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4">
            ...
            />
            {ggAuthPasswordError && (
              <p className="text-red-500 text-sm mb-4">{ggAuthPasswordError}</p>
            )}
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700 mb-8">
          ...
          >
            {t('security.google_authenticator')}
          </button>
        </div>
          ...
          <div className="animate-fadeIn">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex justify-center">
                  <button
                    ...
                    className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md disabled:opacity-60 disabled:cursor-not-allowed whitespace-nowrap"
                  >
                    {isSendingCode ? (
                      <div className="flex items-center">
              ...
              <button
                ...
                disabled={isChangingPassword || !newPassword || !confirmPassword || !isCodeSent || verificationCode.some(code => !code)}
